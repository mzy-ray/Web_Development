    ...
    async.each(queries, function(query, callback) {
        https.get(query.url, function(response){
            ...
            if (response.statusCode === 200){
                ...
                response.on('end', function() {
                    queryData.push({[query.name]: JSON.parse(data)});
                    callback(null, response);
                });
            }
        }).on('error', function(err) {
            queryData.push({[query.name]: err.message});
        });
    }, function(err) {
        res.json(queryData);
    });    
...
app.post('/complete', function (req, res) {
    let url = 'http://dev.markitondemand.com/MODApis/Api/v2/Lookup/json?input=' + req.body;
